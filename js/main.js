...
function produceProjectItemPage() {
	...
	const project = projectCollection.projects.find(proj => proj[pIDKey] === paramProjectID);
...
function produceItemContent(projectJSON) {
	...
	if (projectJSON[pAdditionalMediaKey].length > 0) {
		...
		for (var i = 0; i < parsedAdditionalMediaFiles.length; i++) {
			...
			newMediaLink.href =
				"projects/" + projectJSON[pIDKey] + "/" + parsedAdditionalMediaFiles[i]; /* OUTPUT: `/projectmedia/95/my_photo.jpg` */
	...
	if (
		projectJSON[sPersonalURLKey] != "" && projectJSON[sPersonalURLKey] != null
	) {
	...
	for (var i = 0; i < projectCollection["projects"].length; i++) {
		...
		for (var t = 0; t < splitTags.length; t++) {
			...
			if (newItem != "" && newItem != null) {
				...
				allTagList.indexOf(newItem) === -1 ? allTagList.push(newItem) : console.log(
